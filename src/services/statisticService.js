        ...
        try {
            if (!data.oneDate && !data.twoDate) {
                ...
                for (let i = 0; i < orderProduct.length; i++) {
                    ...
                    let importPrice = 0
                    for (let j = 0; j < orderProduct[i].orderDetail.length; j++) {
                        // Không còn bảng ReceiptDetail nên không thể tính giá nhập
                        // Sử dụng giá bán làm giá nhập tạm thời
                        orderProduct[i].orderDetail[j].importPrice = orderProduct[i].orderDetail[j].realPrice * 0.7; // Giả sử giá nhập là 70% giá bán
                        importPrice = importPrice + (orderProduct[i].orderDetail[j].importPrice * orderProduct[i].orderDetail[j].quantity);
                        totalprice = totalprice + (orderProduct[i].orderDetail[j].realPrice * orderProduct[i].orderDetail[j].quantity);
                    }
                ...
                for (let i = 0; i < orderProduct.length; i++) {
                    totalPrice = totalPrice + orderProduct[i].totalpriceProduct
                    totalImportPrice = totalImportPrice + orderProduct[i].importPrice
                    totalProfitPrice = totalProfitPrice + orderProduct[i].profitPrice
                }
                resolve({
                    errCode: 0,
                    data: {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < res.rows.length; i++) {
                ...
                let receiptDetail = []
                let orderDetail = []
                res.rows[i].productDetaildData = await db.ProductDetail.findOne({ where: { id: res.rows[i].productdetailId } })
                ...
                // Không còn bảng ReceiptDetail nên không thể tính số lượng nhập
                // Sử dụng số lượng cố định làm số lượng nhập tạm thời
                quantity = 100; // Giả sử mỗi sản phẩm có 100 đơn vị
                
                orderDetail = await db.OrderDetail.findAll({ where: { productId: res.rows[i].id } })
...
module.exports = {
    ...
    getStatisticStock: getStatisticStock
}
